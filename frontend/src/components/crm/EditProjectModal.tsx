    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 font-mono">
      <div className="bg-panel-bg rounded-lg border border-brand-primary/30 shadow-[0_0_40px_rgba(16,185,129,0.15)] w-full max-w-4xl max-h-[90vh] overflow-y-auto custom-scrollbar">
        <div className="flex justify-between items-center p-6 border-b border-panel-border sticky top-0 bg-panel-bg/95 backdrop-blur z-10">
          <h2 className="text-2xl font-bold tracking-widest uppercase"><span className="text-brand-purple">EDITAR_</span><span className="text-brand-orange">PROYECTO</span></h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input required name="companyName" type="text" defaultValue={project.companyName} className="w-full p-3 bg-black/50 border border-panel-border rounded text-white focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all placeholder-gray-600" placeholder="Ej. ACME Corp" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-widest">Tamaño</label>
                  <select name="companySize" defaultValue={project.companySize || ""} className="w-full p-3 bg-black/50 border border-panel-border rounded text-white focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all appearance-none">
                ...
                <div>
                  ...
                  <input name="industry" type="text" defaultValue={project.industry || ""} className="w-full p-3 bg-black/50 border border-panel-border rounded text-white focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all placeholder-gray-600" placeholder="Ej. Tecnología" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select required name="contactChannel" defaultValue={project.contactChannel} className="w-full p-3 bg-black/50 border border-panel-border rounded text-white focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all appearance-none">
                ...
                <div>
                  ...
                  <select required name="status" defaultValue={project.status} className="w-full p-3 bg-black/50 border border-panel-border rounded text-white focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all appearance-none">
            ...
            <div className="space-y-4 md:col-span-2">
                  ...
                  <>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input name="recurringRevenue" type="number" step="0.01" defaultValue={project.recurringRevenue || ""} className="w-full p-3 pl-8 bg-black/50 border border-panel-border rounded text-white focus:border-brand-purple focus:ring-1 focus:ring-brand-purple outline-none transition-all placeholder-gray-700" placeholder="0.00" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-brand-purple mb-2 uppercase tracking-widest">Frecuencia Licencia</label>
                      <select name="recurringFrequency" defaultValue={project.recurringFrequency || "MONTHLY"} className="w-full p-3 bg-black/50 border border-panel-border rounded text-white focus:border-brand-purple focus:ring-1 focus:ring-brand-purple outline-none transition-all appearance-none">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-3 text-brand-purple/50">$</span>
                    <input name="quotedPrice" type="number" step="0.01" defaultValue={project.quotedPrice || ""} className="w-full p-3 pl-8 bg-black/50 border border-panel-border rounded text-white focus:border-brand-purple focus:ring-1 focus:ring-brand-purple outline-none transition-all placeholder-gray-700" placeholder="0.00" />
                  </div>
                </div>
                <div>
            ...
            <div className="space-y-4 md:col-span-2">
              ...
              <div>
                <textarea name="projectNotes" rows={4} defaultValue={project.projectNotes || ""} className="w-full p-4 bg-black/50 border border-panel-border rounded text-white focus:border-brand-orange focus:ring-1 focus:ring-brand-orange outline-none resize-none transition-all placeholder-gray-600" placeholder=">_ Ingresar requerimientos..."></textarea>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-4 pt-6 border-t border-panel-border sticky bottom-0 bg-panel-bg/95 backdrop-blur pb-2">
            ...
            <button type="submit" disabled={isSubmitting} className="px-6 py-2 border border-brand-primary bg-brand-primary/10 text-brand-primary rounded hover:bg-brand-primary hover:text-white hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all disabled:opacity-50 font-bold tracking-widest uppercase text-sm">
              {isSubmitting ? 'PROCESANDO...' : '> GUARDAR_CAMBIOS'}
            </button>
          </div>
        </form>
      </div>
